你已获得 **5 个揭榜挂帅专属设备套餐激活码**，数量与通知中的“5个设备套餐”一致，套餐有效期为6个月。需要注意，我只能确认激活码数量和格式，**不能仅凭通知判断激活码是否已经成功绑定、是否仍有效**。官方([萤石社区][1])成功，且每条记录的 `activeCode=0`，才能确认套餐激活完成。官方接口支持一次提交数组，通道号通常为 `1`。([萤石社区][1])按敏感信息管理，不要放入方案、前端代码、Git仓库或直接写入编程助手生成的源代码。下面的开发方案不会回显你提供的完整激活码，实际使用时应放入服务器本地环境变量或独立密钥文件。

新版方案严格按照官方资源边界设计：核心设备接入和音视频能力通过萤石开放平台 API/SDK 实现；AI模型在团队自己的服务器上运行；基础版本不依赖云录制、官方AI等额外付费服务。官方允许团队自行选择 Python、PyTorch 等开发环境，但要求完成设备接入，并原则上通过萤石 API/SDK 实现核心音视频功能；官方也不提供训练数据集，赛事设备不直接支持算法训练平台。([萤石社区][2])riant="document" id="73164"}
...
**当前资源：** 5个萤石赛事专属设备套餐，套餐有效期6个月
**版本：** V2.0
**设计原则：** 基础设备权益即可运行，增值服务按需启用
...
本项目当前不应把主要时间投入萤石云端AI或复杂多模态服务，而应优先完成以下核心能力：
...
该策略能够最大限度利用已经获得的5个设备套餐，同时控制软件费用、开发复杂度和项目风险。
:::

萤石官方的新手流程明确包含设备接入、设备管理、直播与回放、设备消息、视频处理和AI等阶段；Web/H5 SDK可承担直播、回放、截图、录制、清晰度切换等播放功能，平台也支持通过API获取RTMP、HLS、HTTP-FLV等标准播放方式。([萤石社区][3])，建议要求其首先完成“项目骨架—Token管理—设备同步—套餐激活—单路播放”五个步骤，再进入AI开发。
